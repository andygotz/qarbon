...
def _get_package_dir():
    return os.path.abspath(_abspath(os.path.pardir, os.path.pardir))
...
def _get_local_extensions():
    local_ext_dir = _abspath(_get_package_dir(), 'doc')
    if not os.path.isdir(local_ext_dir):
        return []
    local_extensions = []
    items = os.listdir(local_ext_dir)
    for item in items:
        if not item.endswith(".py"):
            continue
        if item.startswith("."):
            continue
        abs_item = os.path.join(local_ext_dir, item)
        # avoid non files
        if not os.path.isfile(abs_item):
            continue
        mod = os.path.splitext(item)[0]
        local_extensions.append(mod)
...
version = '.'.join(_release.version.split('.')[:2])
...
latex_documents = [
  ('contents', '{0}.tex'.format(_name), '{0} Documentation'.format(_Name),
   '{0} team'.format(_Name), 'manual'),
]

# The name of an image file (relative to this directory) to place at the top of
# the title page.
latex_logo = '_static/{0}_screenshot.png'.format(_name)
